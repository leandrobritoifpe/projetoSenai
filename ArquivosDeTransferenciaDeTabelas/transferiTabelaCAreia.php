...
while ($registro = mssql_fetch_array($resultado)) {
    ...
    $arrayDados[] = $registro['FOTODEICONE'];
    $arrayDados[] = $registro['COR'];
    $arrayDados[] = $registro['VISUALIZAVEL'];
...
$tabelaDestino = 'PLANESCOLAR.dbo.PHE_C_AREA';
...
for ($index = 0; $index < count($listaDeDados); $index++) {
    ...
    for ($i = 0; $i < count($arrayCopiaDeDados); $i++) {
       $arrayCopiaDeDados[$i];
        $insert = "INSERT $tabelaDestino (ID,CODIGO,NOME,DESCRICAO,STATUS,FOTODEICONE,COR,VISUALIZAVEL) "
               ."VALUES ($arrayCopiaDeDados[0],'$arrayCopiaDeDados[1]','$arrayCopiaDeDados[2]','$arrayCopiaDeDados[3]','$arrayCopiaDeDados[4]',"
               ."'$arrayCopiaDeDados[5]','$arrayCopiaDeDados[6]',$arrayCopiaDeDados[7])";
