...
class CalendarioSegSab {
    ...
    public function verificaData($codFilial, $mes, $dia,$turno,$ano){
        ...
        if ($tamandoDaString != 2 && $tamandoDaStringMes != 2) {
             $select = "SELECT * FROM PHE_CALENDARIO_ESCOLA WHERE FNL_SS = 1 AND DATADIA LIKE '$ano-0$mes-0$dia' AND CODFILIAL = $codFilial AND CODTURNO = $turno";
        ...
        elseif($tamandoDaString == 2 && $tamandoDaStringMes != 2){
             $select = "SELECT DATADIA FROM PHE_CALENDARIO_ESCOLA WHERE FNL_SS = 1 AND DATADIA LIKE '$ano-0$mes-$dia' AND CODFILIAL = $codFilial AND CODTURNO = $turno";
             $resultado = mssql_query($select);
             while ($linha = mssql_fetch_array($resultado)) {
                 $cont ++;
             }
        ...
        elseif($tamandoDaString == 2 && $tamandoDaStringMes == 2){
             $select = "SELECT DATADIA FROM PHE_CALENDARIO_ESCOLA WHERE FNL_SS = 1  AND DATADIA LIKE '$ano-$mes-$dia' AND CODFILIAL = $codFilial AND CODTURNO = $turno";
             $resultado = mssql_query($select);
             while ($linha = mssql_fetch_array($resultado)) {
                 $cont ++;
             }
